...
class TestAuthMiddleware:
    ...
    def test_valid_token_passes(self, client, auth_headers):
        r = client.post("/api/ingest/events", json={"errors": [], "console_events": [], "network_events": []},
...
class TestIngestEvents:
    ...
    def test_ingest_network(self, client, auth_headers, store):
        r = client.post("/api/ingest/events", json={
            "errors": [],
            "console_events": [],
            "network_events": [make_network(), make_network(method="POST", url="http://localhost:3000/api/submit", status=201)],
        }, headers=auth_headers)
        assert r.status_code == 200
